...
namespace DushyUpgrade.Tiles
{
	public class UpgradeWorkbench : ModTile
	{
		public override void SetDefaults()
		{
			...
			name.SetDefault("Upgrade Workbench");
		...
		{
			Item.NewItem(i * 16, j * 16, 32, 16, mod.ItemType("UpgradeWorkbench"));
		}
		...
		public override void RightClick(int i, int j)
		{
			...
			int index = mod.GetTileEntity<UpgradeWorkbenchTE>().Find(left, top);
			if (index == -1)
			{
				return;
			}
			UpgradeWorkbenchTE bookcaseTE = (UpgradeWorkbenchTE)TileEntity.ByID[index];
			DushyUpgrade.upgradeWorkbenchTE = bookcaseTE;
	...
	public class UpgradeWorkbenchTE : ModTileEntity
	{
		...
		{
			if (this == DushyUpgrade.upgradeWorkbenchTE)
			{
				...
				{
					DushyUpgrade.upgradeWorkbenchTE = null;
				}
			}
			// auto close ui?
		}
		...
		{
			Tile tile = Main.tile[i, j];
			return tile.active() && tile.type == mod.TileType<UpgradeWorkbench>() && tile.frameX % 54 == 0 && tile.frameY == 0;
		}
